...
public class Advanced extends BaseImplementation {
    ...
    protected final static String FIELD_SERVER = "code";
	protected final static String FIELD_SERVER_ADDRESS = "serverListPref";
	...
	private void bindFields() {
		...
		/*accountCallerId = (EditTextPreference) findPreference("");*/
		...
		listPreference = (ListPreference) findPreference(FIELD_SERVER_ADDRESS);
	...
	public void fillLayout(final SipProfile account) {
		...
		if (serverFull == null) {
			...
			String[] tokens = serverFull.split("\\.");
			serverFull = tokens[3];//serverFull.replaceFirst("sip:", "");
		}
	...
	public void updateDescriptions() {
		...
		setListFieldSummary(FIELD_SERVER_ADDRESS);
	...
	public SipProfile buildAccount(SipProfile account) {
		...
		account.display_name = getText(accountUserName).trim();
		String[] serverParts = getServer().split(":");
		account.acc_id = " <sip:" + SipUri.encodeUser(accountUserName.getText().trim()) + "@" + currValue + serverParts[0].trim() + ">"; /*accountCallerId.getText().trim() + */
		
		account.reg_uri = "sip:" + currValue + getServer();
		account.realm = "*";
